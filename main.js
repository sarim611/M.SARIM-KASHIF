...
//After opening popup, if you click outside the popoup, it will close.
...
function filter() {
    ...
    for (i = 0; i < ProductName.length; i++) {
        ...
        //In search if typed string matches withthe element name.
    ...
    while (switching) {
        ...
        for (i = 0; i < ProductName.length - 1; i++) {
            ...
            if (Number(b) > Number(c)) {
                shouldSwitch = true;
                break; // Break the loop if a switch is needed
            }
        }
        // Each product element will switch with the next product element based on product price sorting
        if (shouldSwitch) {
            ProductName[i].parentNode.insertBefore(ProductName[i + 1], ProductName[i]);
            switching = true; // Set switching to true to continue the loop
        }
    }
}
